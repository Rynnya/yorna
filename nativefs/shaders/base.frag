...
vec3 LambertDiffuse(vec3 radiance, float surfaceLightAngle) {
    return (1.0 / M_PI) * radiance * surfaceLightAngle;
}
...
float Beckmann(PBRData data) {
    ...
    return exp((surfaceHalfAngleSquared - 1.0) / max(roughnessAngle, 1e-6)) / max(M_PI * roughnessAngle * surfaceHalfAngleSquared, 1e-6);
}
...
vec3 calculatePBR(PBRData data) {
    ...
    for (uint i = 0; i < lightUbo.size; i++) {
        ...
        diffuse += 
            (1.0 - FresnelSchlick(F0, data.surfaceLightAngle)) * 
            (1.0 - FresnelSchlick(F0, data.surfaceViewAngle)) *
